...
export const isCurrentTimeInRange = (fromDate: string | null, toDate: string | null): boolean => {
  const currentTime = new Date();

  if (fromDate === null && toDate === null)  return false;

  const _fromDate = fromDate ? new Date(fromDate) : null;
  const _toDate = toDate ? new Date(toDate) : null;

  if (_fromDate === null && _toDate === null) return false;

  if ((_fromDate === null || currentTime >= _fromDate) && (_toDate === null || currentTime <= _toDate)) return true;
